...
export default function ComputerAreasConfig() {
  const navigate = useNavigate();

  const [areas, setAreas] = useState<ComputerArea[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/computer-areas/catalog');
  ...
  const openEditDialog = (area: ComputerArea) => {
    setDialog({
      open: true,
      id: area.id,
      name: area.name,
  ...
  const saveArea = async () => {
    ...
    try {
      setSaving(true);
      if (dialog.id) {
        await api.put(`/computer-areas/catalog/${dialog.id}`, {
          name,
          sortOrder,
          isActive: dialog.isActive,
        });
      } else {
        await api.post('/computer-areas/catalog', {
          name,
          sortOrder,
        });
      }
  ...
  const removeArea = async (area: ComputerArea) => {
    ...
    try {
      await api.delete(`/computer-areas/catalog/${area.id}`);
      await fetchData();
    } catch (error) {

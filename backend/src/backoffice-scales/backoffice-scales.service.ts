...
@Injectable()
export class BackofficeScalesService {
  ...
  private isMissingBackofficeScaleTable(error: unknown): boolean {
    ...
    return tableName.includes('BackofficeScaleAreaCatalog') || tableName.includes('RequisitionBackofficeScaleArea');
  }
  ...
  async findCatalog() {
    try {
      return await this.prisma.backofficeScaleAreaCatalog.findMany({
  ...
  async createArea(body: { name: string; sortOrder?: number }) {
    ...
    let existing: any;
    try {
      existing = await this.prisma.backofficeScaleAreaCatalog.findFirst({
    ...
    if (existing) {
      ...
      return this.prisma.backofficeScaleAreaCatalog.update({
    ...
    const maxOrder = await this.prisma.backofficeScaleAreaCatalog.aggregate({
      _max: { sortOrder: true },
    });
    const nextOrder = (maxOrder._max.sortOrder ?? -1) + 1;

    return this.prisma.backofficeScaleAreaCatalog.create({
      data: {
        name,
  ...
  async updateArea(id: string, body: { name?: string; isActive?: boolean; sortOrder?: number }) {
    let current: any;
    try {
      current = await this.prisma.backofficeScaleAreaCatalog.findUnique({ where: { id } });
    } catch (error) {
      if (this.isMissingBackofficeScaleTable(error)) {
        this.throwMissingMigration();
      }
    ...
    const duplicated = await this.prisma.backofficeScaleAreaCatalog.findFirst({
      where: {
        id: { not: id },
